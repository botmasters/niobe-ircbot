...
var telegram = {
	...
	*/
	...
	setupEvents : function (server) {
		telegram._bot.on('text', (function (server) { return function (msg) {
...
var telegramModule = {
	...
	commands : {
		chan : {
			'!telegram' : { level : 50 , callback : telegram.cmdMain }
		}
	},
	...
	initModule : function (server, next) {
		telegram._bot = new TelegramBot(config.telegram.token, {polling: true});
