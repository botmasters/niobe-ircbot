...
var urlInfo = {
	...
	*/
	...
	youtubeInfo : function (server, target, videoId) {
		youtube.videos.list({key: config.google.apiKey, part: 'id,contentDetails,statistics,snippet', id: videoId}, function (err, data) {
			if (err) {
	...
	detectService : function (url) {
		...
		Object.keys(urlInfo._serviceDetectors).forEach(function (serviceDetector) {
			...
			if (!detectedService && serviceMatch) {
				detectedService = { name: serviceDetector, parts : serviceMatch };
			}
		});
